...
class MultiviewData(Dataset):
    def __init__(self, db, device, path="datasets/"):
        ...
        if db == "MSRCv1":
            mat = sio.loadmat(os.path.join(path, 'MSRCv1_missing_0.5.mat'))
            X_data = mat['X']
            ...
            mat = sio.loadmat(os.path.join(path, 'MNIST_USPS_missing_0.5.mat'))
            ...
            mat = sio.loadmat(os.path.join(path, 'BDGP_missing_0.5.mat'))
            X1 = mat['X1'].astype(np.float32)
            X2 = mat['X2'].astype(np.float32)
            ...
            mat = sio.loadmat(os.path.join(path, 'Fashion_missing_0.5.mat'))
